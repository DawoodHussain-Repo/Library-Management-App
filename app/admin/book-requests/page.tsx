        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-gray-500">
                ...
                <tbody>
                    {records.map((record) => (
                        <tr key={record.id} className="border-b bg-white hover:bg-gray-50">
                            <td className="px-6 py-4">
                                <span className="block font-medium text-gray-900">{record.user}</span>
                                <span className="block text-xs text-gray-500">{record.email}</span>
                            </td>
                            <td className="px-6 py-4 flex items-center gap-3">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={record.cover!} alt={record.book!} className="w-8 h-10 object-cover rounded-sm" />
                    ...
                    {records.length === 0 && (
                        <tr>
                            <td colSpan={5} className="px-6 py-4 text-center text-gray-500">No borrow records found.</td>
                        </tr>
                    )}
                </tbody>
            </table>
